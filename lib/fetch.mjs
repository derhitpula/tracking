...
export async function fetchHtml(url) {
  ...
  // Cloudflare-Sperre erkannt: FlareSolverr (löst JS-Challenge, nutzt Proxy intern);
  // ohne FlareSolverr nur den Proxy versuchen (reicht aber bei JS-Challenges nicht).
  if (CF_MARK.test(html)) {
    const fns = process.env.FLARESOLVERR_URL ? [viaSolver] : [viaProxy];
    for (const fn of fns) {
