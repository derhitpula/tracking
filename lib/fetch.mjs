...
export async function fetchHtml(url) {
  ...
  // Cloudflare-Sperre erkannt -> über FlareSolverr erneut versuchen
  if (CF_MARK.test(html)) {
    try { const solved = await viaSolver(url); if (solved && !CF_MARK.test(solved)) return solved; } catch { /* gibt unten die Roh-Seite zurück */ }
  }
  return html;
}
