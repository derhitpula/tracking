...
const FLIP = { '1': '2', '2': '1' };
// kanonische 1X2-Codes -> interne betmonitor-Keys
const CODE2KEY = { HOME_WIN: '1', DRAW: 'X', AWAY_WIN: '2' };

// Explizit früh aufrufen (z.B. am Anfang von collect) um den Cache zu befüllen
// bevor Spiele starten und von betmonitor aus /today verschwinden.
export async function prefetchToday() { await fetchToday(); }

export async function betmonitorOdds(home, away, date, marketCode) {
  const code = CODE2KEY[marketCode];
  if (!code) return null; // betmonitor liefert nur 1X2
