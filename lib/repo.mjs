...
export function setTipOdds(db, tipId, odds) {
  db.prepare('UPDATE tips SET odds=?, updated_at=? WHERE id=?').run(odds ?? null, nowIso(), tipId);
}

export function setClosingOdds(db, tipId, odds) {
  db.prepare('UPDATE tips SET closing_odds=?, updated_at=? WHERE id=?').run(odds, nowIso(), tipId);
}

// Tipps deren Spiel angestoßen ist, aber noch keine Schlussquote eingefroren wurde.
export function tipsNeedingClosing(db) {
  return db.prepare(`SELECT t.id, t.ref_odds, t.odds FROM tips t
    JOIN matches m ON m.id=t.match_id
    WHERE t.closing_odds IS NULL AND m.kickoff IS NOT NULL AND m.kickoff <= ?
      AND (t.ref_odds IS NOT NULL OR t.odds IS NOT NULL)`).all(nowIso());
}
...
export function dashboardRows(db) {
  return db.prepare(`
    SELECT s.key AS source, m.home_team AS home, m.away_team AS away,
      ...
      t.market_code, t.selection, t.odds, t.ref_odds, t.closing_odds,
