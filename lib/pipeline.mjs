...
import { normalizeMarket, fromLegacy, settleTip, profitUnits } from './markets.mjs';
  ...
  for (const a of list) {
    ...
    for (const t of tips) {
      ...
      const matchId = (t.home && t.away)
        ...
        : null;
      // Adapter mit strukturiertem Code (soccervista/betmines) -> direkt mappen,
      // sonst Freitext normalisieren.
      const code = (t.market && fromLegacy(t.market)) || normalizeMarket(t.market_raw, t.home, t.away);
...
export async function enrich(db) {
  ...
  for (const m of matches) {
    ...
    if (!info) continue;
    try { repo.updateMatchResult(db, m.id, info); } catch { continue; }
...
export function settle(db) {
  ...
  for (const t of tips) {
    ...
    if (!result) continue;
    // Gewinn auf Eigenquote; fehlt sie (z. B. SoccerVista), Referenzquote nutzen.
    const odds = t.odds ?? t.ref_odds;
    repo.recordSettlement(db, t.id, result, profitUnits(result, odds), 'api');
...
// --- freezeClosing: Schlussquote bei Anpfiff einfrieren (für CLV) -----------
// closing_odds = letzte bekannte Marktquote (ref_odds), sobald das Spiel
// angestoßen ist. Daraus berechnet das Dashboard den Closing Line Value.
export function freezeClosing(db) {
  const tips = repo.tipsNeedingClosing(db);
  if (!tips.length) { log('Keine Schlussquoten einzufrieren.'); return; }
  let n = 0;
  for (const t of tips) {
    const c = t.ref_odds ?? t.odds;
    if (c != null) { repo.setClosingOdds(db, t.id, c); n++; }
  }
  log(`  ${n} Schlussquoten eingefroren.`);
}
...
export function prune(db, seen = {}) {
  ...
  const dead = db.prepare(`
    ...
    JOIN tips t ON t.raw_tip_id=r.id
    JOIN matches m ON m.id=t.match_id
    WHERE t.market_code IS NULL AND m.kickoff IS NOT NULL
      AND m.kickoff < datetime('now', '-24 hours')`).all();
