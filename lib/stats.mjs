...
export function toUnits(rows) {
  ...
  for (const t of rows) {
    ...
    if (t.slip_ref) {
      ...
      // Einzelwette: einheitliche Referenzquote bevorzugen, sonst Eigenquote.
      units.push({ kind: 'single', source: t.source, odds: t.ref_odds ?? t.odds,
        market: t.market, match_date: t.match_date, result: res, legs: [t] });
    }
  }
...
export function marketStats(rows) {
  ...
  for (const t of rows) {
    ...
    add(byMarket.get(k), { result: t.result || 'pending', odds: t.ref_odds ?? t.odds });
  }
  return byMarket;
}
