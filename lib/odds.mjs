...
function mapping(code) {
  let m;
  if (code === 'HOME_WIN') return [/^match winner$/i, /^home$/i];
  if (code === 'DRAW') return [/^match winner$/i, /^draw$/i];
  if (code === 'AWAY_WIN') return [/^match winner$/i, /^away$/i];
  if (code === 'DC_1X') return [/double chance/i, /home\/draw/i];
  if (code === 'DC_12') return [/double chance/i, /home\/away/i];
  if (code === 'DC_X2') return [/double chance/i, /draw\/away/i];
  if (code === 'BTTS_YES') return [/both teams.*score/i, /^yes$/i];
  if (code === 'BTTS_NO') return [/both teams.*score/i, /^no$/i];
  if ((m = code.match(/^OVER_([0-5])_5$/))) return [/goals over\/under/i, new RegExp(`^over ${m[1]}\\.5$`, 'i')];
  if ((m = code.match(/^UNDER_([0-5])_5$/))) return [/goals over\/under/i, new RegExp(`^under ${m[1]}\\.5$`, 'i')];
  return null;
}

const FLIP = { HOME_WIN: 'AWAY_WIN', AWAY_WIN: 'HOME_WIN', DC_1X: 'DC_X2', DC_X2: 'DC_1X' };
...
function pickToddsOdds(events, home, away, code) {
  ...
  if (!h2h) return null;
  // Outcome-Mapping: HOME_WIN=home_team, AWAY_WIN=away_team, DRAW=Draw
  const targetName = effectiveCode === 'DRAW' ? 'Draw'
    : effectiveCode === 'HOME_WIN' ? ev.home_team : ev.away_team;
  const outcome = h2h.outcomes.find((o) =>
    effectiveCode === 'DRAW' ? o.name === 'Draw' : simScore(o.name, targetName) > 0.45,
  );
...
async function theoddsapiOdds(home, away, date, code) {
  if (!TODDS_KEY || !date || !['HOME_WIN', 'DRAW', 'AWAY_WIN'].includes(code)) return null;
