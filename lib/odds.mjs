...
async function fixtureBets(fixtureId) {
  ...
  if (!KEY) return null;
  const res = await fetch(`https://v3.football.api-sports.io/odds?fixture=${fixtureId}&bookmaker=${BOOKMAKER}`,
    { headers: { 'User-Agent': UA, 'x-apisports-key': KEY } });
  if (!res.ok) return null;
  const j = await res.json();
  const bets = j.response?.[0]?.bookmakers?.[0]?.bets || [];
...
// Referenzquote für Fixture + Markt-Code (oder null)
export async function referenceOdds(fixtureId, code) {
