...
const NAMEALIAS = {
  ...
  turkei: 'turkey', tschechien: 'czech republic', niederlande: 'netherlands',
};
...
// Passendes API-Football-Fixture (auch vor Anpfiff, ohne Score-Pflicht) finden.
// Liefert das Spielobjekt inkl. id – für die Referenzquoten genutzt.
export async function findApiFixture(home, away, date) {
  if (!APIFOOTBALL_KEY || !date) return null;
  let games; try { games = await apiFootballDay(date); } catch { return null; }
