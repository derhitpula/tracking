...
// Teamnamen-Normalisierung & Fuzzy-Vergleich -> lib/names.mjs (geteilt).
import { nameScore, TEAM_OK } from './names.mjs';
...
async function sportsdbDay(date) {
  ...
  const games = (j.events || []).map((e) => ({
    ...
    kickoff: e.strTimestamp || (e.dateEvent && e.strTime ? `${e.dateEvent}T${e.strTime}Z` : null),
...
export async function findApiFixture(home, away, date) {
  ...
  return m ? { id: m.g.id, swapped: m.swapped, kickoff: m.g.kickoff } : null;
}

// Nur die Anstoßzeit suchen (auch ohne Score). Erst API-Football (falls Kontingent
// übrig), dann TheSportsDB (keyless) als Fallback. Liefert ISO-Kickoff oder null.
export async function findKickoff(home, away, date) {
  if (!date) return null;
  // 1) API-Football
  try {
    const m = await scanApi(home, away, date);
    if (m?.g?.kickoff) return m.g.kickoff;
  } catch {}
  // 2) TheSportsDB im Datumsfenster
  for (const off of DATE_OFFSETS) {
    let games; try { games = await sportsdbDay(shiftDate(date, off)); } catch { continue; }
    if (!games) continue;
    let best = null, bestScore = 0;
    for (const g of games) {
    ...
    if (best && bestScore >= TEAM_OK) return best.kickoff;
  }
  return null;
}

// Vollständige Match-Daten (Anstoß, Status, Endstand, Fixture-ID) in einem Lookup.
// Für die Anreicherung auf MATCH-Ebene: ein API-Call pro Spiel statt pro Tipp.
// Endstand ist orientierungs-korrekt (home/away) zurückgegeben.
export async function lookupMatch(home, away, date) {
  if (!date) return null;
  // 1) API-Football (Fixture inkl. Status/Endstand/Anstoß)
  try {
    const m = await scanApi(home, away, date);
    if (m) {
      const { g, swapped } = m;
      const finished = g.finished && g.fh != null && g.fa != null;
      return {
        apiMatchId: g.id ?? null, provider: 'apifootball', kickoff: g.kickoff ?? null,
        status: finished ? 'finished' : 'scheduled',
        fh: finished ? (swapped ? g.fa : g.fh) : null,
        fa: finished ? (swapped ? g.fh : g.fa) : null,
        hh: finished && g.hh != null ? (swapped ? g.ha : g.hh) : null,
        ha: finished && g.hh != null ? (swapped ? g.hh : g.ha) : null,
      };
    }
  } catch {}
  // 2) TheSportsDB (keyless Fallback)
  for (const off of DATE_OFFSETS) {
    let games; try { games = await sportsdbDay(shiftDate(date, off)); } catch { continue; }
    if (!games) continue;
    let best = null, bestScore = 0, bestSwap = false;
    for (const g of games) {
      const direct = Math.min(nameScore(home, g.home), nameScore(away, g.away));
      const swap = Math.min(nameScore(home, g.away), nameScore(away, g.home));
      const s = Math.max(direct, swap);
      if (s > bestScore) { bestScore = s; best = g; bestSwap = swap > direct; }
    }
    if (best && bestScore >= TEAM_OK) {
      const finished = best.finished && best.fh != null && best.fa != null;
      return {
        apiMatchId: null, provider: 'sportsdb', kickoff: best.kickoff ?? null,
        status: finished ? 'finished' : 'scheduled',
        fh: finished ? (bestSwap ? best.fa : best.fh) : null,
        fa: finished ? (bestSwap ? best.fh : best.fa) : null,
        hh: null, ha: null,
      };
    }
  }
